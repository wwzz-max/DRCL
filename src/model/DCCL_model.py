...
class dccl_encoder(nn.Module):
    ...
    def __init__(self,
        ...
        super(dccl_encoder, self).__init__()
        ...
        #self.SelfAttention = SelfAttention(self.latent_dim)
        ...
        self.recon = Reconstructor(in_channels=self.in_channels,
                                patch_size=self.patch_size,
                                latent_dim=self.latent_dim)
        self.patch_recon = PatchRecon(in_channels=self.in_channels,
                                patch_size=self.patch_size,
                                latent_dim=self.latent_dim)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        if not self.inference:
            ...
            patch_recon = self.patch_recon(z_anchors)
            # global reconstruction
            global_recon = self.recon(z)
            return x, z, global_recon, patch_real, patch_recon, z_anchors, z_positives
        else:
            return z
...
class Reconstructor(nn.Module):
    ...
    Maps from [B, S, L] to [B, S, C, P, P] for patches and [B, L, H, W] to [B, C, H, W] for global images.
    """

    def __init__(self, in_channels: int = 3, num_kernels: int = 64, latent_dim: int = None, patch_size: int = 5):
    ...
    def global_recon(self, z): 
        return self._reconstruct(z)


    def _reconstruct(self, z):
        x = self.conv1(z)
        #x = F.leaky_relu(self.conv1(z))
        return x

    def forward(self, z):
        if len(z.shape) == 4:  # Global reconstruction
            return self.global_recon(z)
        else:  # Patch reconstruction
            return self.patch_recon(z)
