...
if __name__ == '__main__':
    ...
    for image_idx in tqdm(range(21,len(np_files_path))):
        ...
        seg_spectranet = label_hint_seg(label_true=label_true,
                                    label_pred=label_spectralnet)
        ...
        scprep.plot.scatter2d(data_phate,
                              c=continuous_renumber(
                                  label_spectralnet.reshape((H * W, -1))),
                              legend_anchor=(1, 1),
                              ax=ax,
                              title='deep Spectral net',
                              xticks=False,
                              yticks=False,
                              label_prefix="PHATE",
                              fontsize=10,
        ...
        scprep.plot.scatter2d(data_phate,
                              c=seg_spectranet.reshape((H * W, -1)),
                              legend_anchor=(1, 1),
                              ax=ax,
                              title='deep Spectral net',
                              xticks=False,
        ...
        ax.imshow(seg_spectranet, cmap='gray')
        ax.set_title('Spectral K-means')
        ax.set_axis_off()
        ax = fig2.add_subplot(1, 4, 4)
        ax.imshow(continuous_renumber(label_spectralnet), cmap='tab20')
        ax.set_title('Spectral K-means')
        ax.set_axis_off()
