...
def load_cuts_kmeans(path: str) -> dict:
    numpy_array = np.load(path)
    hashmap = {}
    hashmap['image'] = numpy_array['image']
    hashmap['label_true'] = numpy_array['label']
    hashmap['latent'] = numpy_array['latent']
    hashmap['label_cuts_kmeans'] = numpy_array['label_kmeans']  
    return hashmap

def load_spectralnet(path: str) -> dict:
    numpy_array = np.load(path)
    hashmap = {}
    hashmap['image'] = numpy_array['image']
    hashmap['label_true'] = numpy_array['label']
    hashmap['latent'] = numpy_array['latent']
    hashmap['label_spectralnet'] = numpy_array['label_spectralnet'] #label_spectralnet  这里得替换
    return hashmap

def load_pos_spectralnet(path: str) -> dict:
    numpy_array = np.load(path)
    hashmap = {}
    hashmap['image'] = numpy_array['image']
    hashmap['label_true'] = numpy_array['label']
    hashmap['latent'] = numpy_array['latent']
    hashmap['label_pos_spectralnet'] = numpy_array['label_pos_spectralnet'] #label_pos_spectralnet
    return hashmap
...
def load_cuts_diffusion(path: str) -> dict:
    numpy_array = np.load(path)
    hashmap = {}
    hashmap['image'] = numpy_array['image']
    hashmap['recon'] = numpy_array['recon']
    hashmap['label_true'] = numpy_array['label']
    hashmap['latent'] = numpy_array['latent']
    hashmap['labels_cuts_diffusion'] = numpy_array['labels_diffusion']
    return hashmap
...
def load_pixel_spectralnet(path: str) -> dict:
    numpy_array = np.load(path)
    hashmap = {}
    hashmap['image'] = numpy_array['image']
    hashmap['label_true'] = numpy_array['label']
    hashmap['latent'] = numpy_array['latent']
    hashmap['label_pixel_spectralnet'] = numpy_array['label_spectralnet']
    return hashmap

def load_pixel_pos_spectralnet(path: str) -> dict:
    numpy_array = np.load(path)
    hashmap = {}
    hashmap['image'] = numpy_array['image']
    hashmap['label_true'] = numpy_array['label']
    hashmap['latent'] = numpy_array['latent']
    hashmap['label_pixel_pos_spectralnet'] = numpy_array['label_pos_spectralnet']
    return hashmap
...
def segment_every_diffusion(hashmap: dict) -> dict:
    label_true = hashmap['label_true']
    labels_pred = hashmap['labels_diffusion']  # cuts_diffusion
...
def segment_every_cuts_diffusion(hashmap: dict) -> dict:
    label_true = hashmap['label_true']
    labels_pred = hashmap['labels_cuts_diffusion']
    ...
    hashmap['segs_cuts_diffusion'] = segs
...
def cuts_persistent_structures(hashmap: dict) -> dict:
    label_true = hashmap['label_true']
    labels_diffusion = hashmap['labels_cuts_diffusion']
    ...
    hashmap['labels_cuts_diffusion'] = labels_diffusion
    hashmap['label_cuts_diffusion-persistent'] = persistent_label
...
def barplots(meta_metrics, metric_name_map, entity_tuples, config_filepaths):
    ...
    for i, key in enumerate(metric_name_map.keys(), start=1):
        ...
        palette_dark = sns.color_palette('Blues',
                                         n_colors=4 * len(method_list))[::-1]
        palette_red = sns.color_palette('Blues',
                                         n_colors=4 * len(method_list))[::-1]
        ...
        if i < num_cols:
            ...
            #ax.legend(loc='upper left', bbox_to_anchor=(1.01, 5))
            ax.legend(loc='upper left', bbox_to_anchor=(1.01, 1))  # 调整图例位置
    fig.tight_layout(pad=2)
    fig.savefig(figure_path)
    return
...
if __name__ == '__main__':
    ...
    for config_file in args.config:
        ...
        files_folder_pixel_spectralnet = '%s/%s' % (config.output_save_path,
                                               'numpy_files_seg_pixel_spectralnet')
        files_folder_pixel_pos_spectralnet = '%s/%s' % (config.output_save_path,
                                               'numpy_files_seg_pixel_pos_spectralnet')
        ...
        np_files_path_cuts_kmeans = sorted(
            glob('%s/%s' % (files_folder_cuts_kmeans, '*.npz')))

        np_files_path_kmeans = sorted(
            glob('%s/%s' % (files_folder_kmeans, '*.npz')))


        np_files_path_spectralnet = sorted(
            glob('%s/%s' % (files_folder_spectralnet, '*.npz')))
 

        np_files_path_pos_spectralnet = sorted(
            glob('%s/%s' % (files_folder_pos_spectralnet, '*.npz')))

        np_files_path_cuts_diffusion = sorted(
        ...
        np_files_path_pixel_spectralnet = sorted(
            glob('%s/%s' % (files_folder_pixel_spectralnet, '*.npz')))
        np_files_path_pixel_pos_spectralnet = sorted(
            glob('%s/%s' % (files_folder_pixel_pos_spectralnet, '*.npz')))
        ...
        num_files = max([
            ...
            len(np_files_path_cuts_kmeans),
            len(np_files_path_kmeans),
            len(np_files_path_spectralnet),
            len(np_files_path_pos_spectralnet),
            len(np_files_path_diffusion),
            len(np_files_path_cuts_diffusion),
            len(np_files_path_pixel_kmeans),
            len(np_files_path_pixel_spectralnet),
            len(np_files_path_pixel_pos_spectralnet),
        ...
        assert len(np_files_path_cuts_kmeans) == num_files or len(
            np_files_path_cuts_kmeans) == 0
        assert len(np_files_path_kmeans) == num_files or len(
            np_files_path_kmeans) == 0
        assert len(np_files_path_spectralnet) == num_files or len(
            np_files_path_spectralnet) == 0
        assert len(np_files_path_pos_spectralnet) == num_files or len(
            np_files_path_pos_spectralnet) == 0
        assert len(np_files_path_cuts_diffusion) == num_files or len(
            np_files_path_cuts_diffusion) == 0
        ...
        assert len(np_files_path_pixel_spectralnet) == num_files or len(
            np_files_path_pixel_spectralnet) == 0
        assert len(np_files_path_pixel_pos_spectralnet) == num_files or len(
            np_files_path_pixel_pos_spectralnet) == 0
        ...
        has_cuts_kmeans = True if len(np_files_path_cuts_kmeans) == num_files else False
        has_kmeans = True if len(np_files_path_kmeans) == num_files else False
        has_spectralnet = True if len(np_files_path_spectralnet) == num_files else False
        has_pos_spectralnet = True if len(np_files_path_pos_spectralnet) == num_files else False
        has_cuts_diffusion = True if len(np_files_path_cuts_diffusion) == num_files else False
        ...
        has_pixel_spectralnet = True if len(np_files_path_pixel_spectralnet) == num_files else False
        has_pixel_pos_spectralnet = True if len(np_files_path_pixel_pos_spectralnet) == num_files else False
        ...
        if has_cuts_kmeans:
            if hparams.is_binary:
                entity_tuples.extend([
                    ('CUTS + Spectral $k$-means', 'label_true', 'seg_cuts_kmeans'),
                ])
            else:
                entity_tuples.extend([
        ...
        #     if hparams.is_binary:
        #         entity_tuples.extend([
        #             ('OURS + Spectral $k$-means', 'label_true', 'seg_kmeans'),
        #         ])
        #     else:
        #         entity_tuples.extend([
        #             ('OURS + Spectral $k$-means', 'label_true',
        #              'label_kmeans'),
        #         ])

        if has_cuts_diffusion:
            print('88888888888888888888888888888888')
            if hparams.is_binary:
                entity_tuples.extend([
                    ('CUTS + Diffusion (persistent)', 'label_true',
                     'seg_cuts_diffusion-persistent'),
                    ('CUTS + Diffusion (best one)', 'label_true',
                     'seg_cuts_diffusion-best'),
                ])
            else:
                entity_tuples.extend([
                    ('CUTS + Diffusion (persistent)', 'label_true',
                     'label_cuts_diffusion-persistent'),
                    ('CUTS + Diffusion (best one)', 'label_true',
                     'label_cuts_diffusion-best'),
                ])
    
        # if has_pixel_spectralnet:
        #     if hparams.is_binary:
        #         entity_tuples.extend([
        #             ('spectralnet', 'label_true', 'seg_pixel_spectralnet'),
        #         ])
        #     else:
        #         entity_tuples.extend([
        #             ('spectralnet', 'label_true', 'label_pixel_spectralnet'),
        #         ])    
        ...
        if has_spectralnet:
            if hparams.is_binary:
                entity_tuples.extend([
                    ('OURS + Spectralnet', 'label_true', 'seg_spectralnet'),   #注意这里
                ])
            else:
                entity_tuples.extend([
                    ('OURS + Spectralnet', 'label_true',
                     'label_spectralnet'),
                ])

        if has_pos_spectralnet:
            if hparams.is_binary:
                entity_tuples.extend([
                    ('OURS + pos + Spectralnet', 'label_true', 'seg_pos_spectralnet'),   #注意这里
                ])
            else:
                entity_tuples.extend([
                    ('OURS + pos + Spectralnet', 'label_true',
                     'label_pos_spectralnet'),
                ])
        ...
        #     if hparams.is_binary:
        #         entity_tuples.extend([
        #             ('OURS + Diffusion (persistent)', 'label_true',
        #              'seg_diffusion-persistent'),
        #             ('OURS + Diffusion (best one)', 'label_true',
        #              'seg_diffusion-best'),
        #         ])
        #     else:
        #         entity_tuples.extend([
        ...
        for image_idx in tqdm(range(num_files)):
            baselines_hashmap, kmeans_hashmap,spectralnet_hashmap,pos_spectralnet_hashmap, diffusion_hashmap = {}, {}, {}, {}, {}
            cuts_kmeans_hashmap, cuts_diffusion_hashmap = {}, {}
            pixel_kmeans_hashmap, pixel_spectralnet_hashmap, pixel_pos_spectralnet_hashmap, pixel_diffusion_hashmap = {}, {}, {}, {}
            ...
            if has_cuts_kmeans:
                cuts_kmeans_hashmap = load_cuts_kmeans(np_files_path_cuts_kmeans[image_idx])

            if has_kmeans:
                kmeans_hashmap = load_kmeans(np_files_path_kmeans[image_idx])
            if has_spectralnet:
                spectralnet_hashmap = load_spectralnet(np_files_path_spectralnet[image_idx])
            if has_pos_spectralnet:
                pos_spectralnet_hashmap = load_pos_spectralnet(np_files_path_pos_spectralnet[image_idx])
            if has_cuts_diffusion:
                cuts_diffusion_hashmap = load_cuts_diffusion(
                    np_files_path_cuts_diffusion[image_idx])
            ...
            if has_pixel_spectralnet:
                pixel_spectralnet_hashmap = load_pixel_spectralnet(np_files_path_pixel_spectralnet[image_idx])
            if has_pixel_pos_spectralnet:
                pixel_pos_spectralnet_hashmap = load_pixel_pos_spectralnet(np_files_path_pixel_pos_spectralnet[image_idx])
            ...
            hashmap = combine_hashmaps(baselines_hashmap, dfc_hashmap,
                                       stego_hashmap, kmeans_hashmap,cuts_kmeans_hashmap,spectralnet_hashmap,pos_spectralnet_hashmap,
                                       diffusion_hashmap,cuts_diffusion_hashmap, pixel_kmeans_hashmap,pixel_spectralnet_hashmap,pixel_pos_spectralnet_hashmap,
                                       pixel_diffusion_hashmap, sam_hashmap,
                                       unet_hashmap, nnunet_hashmap)   #pixel_pos_spectralnet_hashmap
            

            # hashmap = combine_hashmaps(baselines_hashmap, dfc_hashmap,
            #                         stego_hashmap, cuts_kmeans_hashmap,
            #                         cuts_diffusion_hashmap, pixel_kmeans_hashmap,pixel_spectralnet_hashmap,pixel_pos_spectralnet_hashmap,
            #                         kmeans_hashmap,spectralnet_hashmap,pos_spectralnet_hashmap,
            #                         sam_hashmap,
            #                         unet_hashmap, nnunet_hashmap)   #pixel_pos_spectralnet_hashmap
            if has_baselines:
            ...
            if has_cuts_kmeans:
                hashmap = segment(hashmap, label_name='cuts_kmeans')  

            if has_kmeans:
                hashmap = segment(hashmap, label_name='kmeans')          
            if has_spectralnet:
                hashmap = segment(hashmap, label_name='spectralnet')
            if has_pos_spectralnet:
                hashmap = segment(hashmap, label_name='pos_spectralnet')
            
            if has_cuts_diffusion:
                hashmap = cuts_persistent_structures(hashmap)
                hashmap = segment(hashmap, label_name='cuts_diffusion-persistent')
                hashmap = segment_every_cuts_diffusion(hashmap)
            ...
            if has_pixel_spectralnet:
                hashmap = segment(hashmap, label_name='pixel_spectralnet')      #seg_pixel_spectralnet
            if has_pixel_pos_spectralnet:
                hashmap = segment(hashmap, label_name='pixel_pos_spectralnet')   #
            ...
            if not hparams.is_binary:
                ...
                # Relabel each of the diffusion labels.
                if has_cuts_diffusion:
                    for i in range(hashmap['labels_cuts_diffusion'].shape[0]):
                        hashmap['labels_cuts_diffusion'][i, ...] = guided_relabel(
                            label_pred=hashmap['labels_cuts_diffusion'][i, ...],
                            label_true=hashmap['label_true'])
            ...
            for (entry, p1, p2) in entity_tuples:
                ...
                if p2 == 'label_cuts_diffusion-best':
                    # Get the best among all diffusion labels.
                    assert not hparams.is_binary
                    metrics['dice'][entry].append(
                        max([
                            per_class_dice_coeff(
                                ...
                                label_pred=hashmap['labels_cuts_diffusion'][i, ...])
                            for i in range(
                                hashmap['labels_cuts_diffusion'].shape[0])
                        ]))
                    metrics['hausdorff'][entry].append(
                        min([
                            per_class_hausdorff(
                    ...
                    metrics['ergas'][entry].append(
                        min([
                            ergas(hashmap['label_true'],
                                  hashmap['labels_cuts_diffusion'][i, ...]) for i in
                            range(hashmap['labels_cuts_diffusion'].shape[0])
                        ]))
                    metrics['rmse'][entry].append(
                        min([
                            rmse(hashmap['label_true'],
                ...
                elif p2 == 'seg_cuts_diffusion-best':
                    # Get the best among all diffusion segmentations.
                    assert hparams.is_binary
                    metrics['dice'][entry].append(
                        max([
                            dice_coeff(
                                label_true=hashmap['label_true'],
                                label_pred=hashmap['segs_cuts_diffusion'][i, ...])
                            for i in range(hashmap['segs_cuts_diffusion'].shape[0])
                        ]))
                    metrics['hausdorff'][entry].append(
                        min([
                            hausdorff(
    ...
    if META_ANALYSIS:
        ...
        #adjusted_barplots(meta_metrics, metric_name_map, entity_tuples, args.config)
